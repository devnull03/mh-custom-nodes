...
def render_keypoint_frames_to_image(
    ...
    for meta in frames_meta:
        if (
            ...
            and "canvas_height" in meta
            and "canvas_width" in meta
        ):
        ...
        if body_kps is None:
            ...
            openpose_dicts.append(
                {"people": [], "canvas_height": cH, "canvas_width": cW}
            )

            padded, remove_pad = safe_resize_image_with_pad(
            ...
            try:
                ...
                for x, y, c in kps_norm:
                    if c <= 0.0:
                        ...
                        repo_keypoints.append(
                            RepoKeypoint(x=float(x), y=float(y), score=float(c), id=-1)
                        )
                ...
                padded, remove_pad = safe_resize_image_with_pad(
                    repo_canvas, detect_resolution, upscale_method
                )
                cropped = remove_pad(padded)
                final = safe_HWC3(cropped)
                images.append(Image.fromarray(final) if output_type == "pil" else final)
                ...
                people_list = [
                    {
                        "pose_keypoints_2d": [
                            float(v) for trip in body_kps for v in trip
                        ],
                        "face_keypoints_2d": [
                            float(v) for trip in face_kps for v in trip
                        ]
                        if face_kps is not None
                        else None,
                        "hand_left_keypoints_2d": [
                            float(v) for trip in left_hand_kps for v in trip
                        ]
                        if left_hand_kps is not None
                        else None,
                        "hand_right_keypoints_2d": [
                            float(v) for trip in right_hand_kps for v in trip
                        ]
                        if right_hand_kps is not None
                        else None,
                    }
                ...
                openpose_dicts.append(
                    {"people": people_list, "canvas_height": cH, "canvas_width": cW}
                )
                continue
            except Exception as e:
                print(f"[MH_RenderPose] dwpose draw failed, falling back: {e}")
